...
// Writer instructions live in .pi/agents/blog-writer.md — not here.
...
async function draftPost(
    ...
    // Load writer instructions from .pi/agents/blog-writer.md
    // Extension = plumbing. Agent file = instructions.
    const agentFile = path.join(ctx.cwd, ".pi", "agents", "blog-writer.md");
    const systemPrompt = fs.existsSync(agentFile)
        ? fs.readFileSync(agentFile, "utf8")
        : "You are a technical blog writer. Write clearly and concisely.";
